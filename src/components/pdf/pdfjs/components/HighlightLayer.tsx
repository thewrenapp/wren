...
import { scaledPositionToViewport, viewportToScaled, viewportToPdf } from "../lib/coordinates";
  ...
  return (
    <div>
      {currentHighlights.map((highlight, index) => {
        ...
        // Compute zoom scale: ratio of current viewport width to stored viewport width
        const currentViewport = viewer.getPageView(pageNumber - 1).viewport;
        const storedWidth = highlight.position.boundingRect.width;
        const zoomScale = storedWidth > 0 ? currentViewport.width / storedWidth : 1;
        ...
        const highlightUtils: HighlightContainerUtils = {
          ...
          viewportToPdfScaled: (rect: LTWHP) => {
            const viewport = viewer.getPageView(
              (rect.pageNumber || pageNumber) - 1,
            ).viewport;

            return viewportToPdf(rect, viewport);
          },
