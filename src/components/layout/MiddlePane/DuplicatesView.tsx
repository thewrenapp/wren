...
export function DuplicatesView() {
  ...
  const [primaryTargets, setPrimaryTargets] = useState<Record<number, number>>({});
  ...
  const handleToggleSelection = (groupIndex: number, entryId: number) => {
    setSelectedEntries((prev) => {
      const currentSelection = prev[groupIndex] ?? [];
      const isSelected = currentSelection.includes(entryId);
      let nextSelection = isSelected
        ? currentSelection.filter((id) => id !== entryId)
        : [...currentSelection, entryId];

      if (nextSelection.length === 0) {
        nextSelection = [entryId];
      }

      setPrimaryTargets((prevTargets) => {
        let nextPrimary = prevTargets[groupIndex];
        if (!isSelected) {
          nextPrimary = entryId;
        } else if (prevTargets[groupIndex] === entryId) {
          nextPrimary = nextSelection[0];
        }
        return { ...prevTargets, [groupIndex]: nextPrimary };
      });

      return { ...prev, [groupIndex]: nextSelection };
    });
  };

  const handleMergeSelected = async (groupIndex: number) => {
    const group = groups[groupIndex];
    const selected = selectedEntries[groupIndex] ?? [];
    const targetId = primaryTargets[groupIndex];
    if (!targetId) return;

    if (selected.length < 2) {
      toast.error('Select at least two entries to merge');
      return;
    }
    ...
    try {
      ...
      if (discardIds.length > 0) {
        await discardDuplicates(targetId, discardIds);
        const count = await getTrashCount();
        setTrashCount(count);
      }
      toast.success('Selected entries merged and remaining duplicates discarded');
  ...
  const handleKeepAndDiscard = async (groupIndex: number) => {
    const group = groups[groupIndex];
    const keepId = primaryTargets[groupIndex];
    if (!keepId) return;

    const discardIds = group.entries.filter((e) => e.id !== keepId).map((e) => e.id);
    ...
    <div className='flex flex-col h-full'>
      ...
      <div className='px-4 py-3 border-b'>
        ...
        <p className='text-xs text-muted-foreground mt-1'>
          Select multiple entries to merge. The last selected entry will be kept.
        </p>
      </div>
          ...
          {groups.map((group, groupIndex) => (
            <div key={groupIndex} className='border rounded-lg overflow-hidden'>
              ...
              <div className='divide-y'>
                {group.entries.map((entry) => (
                  <DuplicateEntryRow
                    key={entry.id}
                    entry={entry}
                    isSelected={(selectedEntries[groupIndex] ?? []).includes(entry.id)}
                    isPrimary={primaryTargets[groupIndex] === entry.id}
                    onSelect={() => handleToggleSelection(groupIndex, entry.id)}
                  />
                ))}
              </div>
              ...
              <div className='bg-muted/30 px-4 py-3 flex items-center gap-2 justify-end border-t'>
                ...
                <Button
                  size='sm'
                  onClick={() => handleMergeSelected(groupIndex)}
                  disabled={processing !== null}
                >
                  {processing === groupIndex ? (
                    <Loader2 className='h-4 w-4 mr-1 animate-spin' />
...
function DuplicateEntryRow({ entry, isSelected, isPrimary, onSelect }: DuplicateEntryRowProps) {
  return (
    <button
      onClick={onSelect}
      className={cn(
      ...
      <div className='flex items-start gap-3'>
        ...
        <div className='flex-1 min-w-0'>
          ...
          <div className='flex items-center gap-3 mt-2 text-xs text-muted-foreground'>
            ...
            <span>
              {entry.attachmentCount} {entry.attachmentCount === 1 ? 'file' : 'files'}
            </span>
          </div>
        </div>
