...
async fn run_incremental_migrations(pool: &SqlitePool) -> Result<()> {
    ...
    // Outbox for pending Firebase changes (shared entries)
    let _ = sqlx::query(
        r#"CREATE TABLE IF NOT EXISTS sync_outbox (
            id INTEGER PRIMARY KEY AUTOINCREMENT,
    ...
    let _ = sqlx::query("CREATE INDEX IF NOT EXISTS idx_sync_outbox_status ON sync_outbox(status)")
        .execute(pool).await;
    ...
    let _ = sqlx::query(
        r#"CREATE TABLE IF NOT EXISTS share_entries (
            share_id TEXT NOT NULL REFERENCES shares(share_id) ON DELETE CASCADE,
            entry_key TEXT NOT NULL,
            PRIMARY KEY (share_id, entry_key)
        )"#
    ).execute(pool).await;

    // User account (local cache of Firebase auth)
    let _ = sqlx::query(
    ...
    // Sharing columns on entries table
    let _ = sqlx::query("ALTER TABLE entries ADD COLUMN share_id TEXT")
        .execute(pool).await;
    let _ = sqlx::query("ALTER TABLE entries ADD COLUMN sharing_role TEXT")
        .execute(pool).await;
    let _ = sqlx::query("ALTER TABLE entries ADD COLUMN origin_user_id TEXT")
        .execute(pool).await;
    let _ = sqlx::query("ALTER TABLE entries ADD COLUMN is_detached INTEGER NOT NULL DEFAULT 0")
        .execute(pool).await;
